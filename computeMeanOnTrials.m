...
templFromI = int32(app.templateFrom.Value/app.sp)+1;
templToI = int32(app.templateTo.Value/app.sp)+1;
PWfromI = int32(app.winPWleft.Value/app.sp)+1;  % window for power computation
PWtoI = int32(app.winPWright.Value/app.sp)+1;
...
blFrom = int32(app.baselineFrom.Value/app.sp)+1;
blTo = int32(app.baselineTo.Value/app.sp)+1;
respFrom = int32(app.respFrom.Value/app.sp)+1;
respTo = int32(app.respTo.Value/app.sp)+1;
...
for (i=1:app.nCh)
    ...
    if (i ~= syCh || ~flagSynch) % if flagSynch is false always do the following
        if ~(flagExCh && i == exCh) 
            ...
            app.templ(i,:) = template; %gab_2018/09/19 -> make handles.templ a matrix adding the "channel" dimension.
            ...
            for ii=1:app.ntemp
                ...
                tpeak = app.sp*double(respFrom+imax);
